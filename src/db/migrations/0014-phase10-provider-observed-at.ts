...
export const phase10ProviderObservedAtStatements = [
  ...
  // A pending/retry row with a persisted scheduler timestamp has a real
  // operational timestamp. Terminal historical rows with NULL stay unknown.
  `UPDATE provider_deliveries SET observed_at = next_attempt_at
    WHERE observed_at IS NULL AND next_attempt_at IS NOT NULL`,
  ...
  `CREATE TRIGGER phase10_export_provider_insert AFTER INSERT ON provider_deliveries
    ...
    VALUES ('provider_deliveries', NEW.id, printf('%s:%s', NEW.attempt_count, NEW.status), NEW.observed_at,
